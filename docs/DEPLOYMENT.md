...
| `APP_SECRET_KEY` | STRONGLY RECOMMENDED | (transitional: derived from `SECRET_KEY`) | `utils/crypto.py` | Fernet (32-byte url-safe base64). Encrypts Aiia + MobilePay refresh tokens at rest. Generate once: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`. **#359:** if unset, the at-rest key is currently *derived from `SECRET_KEY`* (a CRITICAL log line fires) — coupling bank-token encryption to the JWT secret. Set a dedicated value (own leak boundary) **plus `APP_SECRET_KEY_PREVIOUS`=`<current SECRET_KEY>`** during cutover so existing 90-day tokens keep decrypting. The `DATABASE_URL` fallback was removed. |
