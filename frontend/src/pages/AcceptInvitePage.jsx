...
export default function AcceptInvitePage() {
  ...
  useEffect(() => {
    if (!token || token.length < 10) {
      setError(t("aiInviteLinkInvalid", "This invite link looks invalid. Ask the business owner to send a fresh one."));
    }
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (password.length < 8) {
      setError(t("aiPasswordMin8", "Password must be at least 8 characters."));
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (err?.response?.status === 410 || code === "invite_expired") {
        setError(t("aiInviteExpired", "This invite has expired. Ask the business owner to send a fresh one."));
        ...
        setError(
          msg ||
            t("aiEmailInUse", "An account with this email already exists. Sign in first and then ask the owner to re-invite you so we can link the accounts.")
        );
      } else {
        setError(msg || t("aiSomethingWentWrong", "Something went wrong. Try again or contact support."));
      }
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-gray-900 px-4 py-12">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-7">
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">
          {t("aiInvitedAsRevisor", "You're invited as a revisor")}
        </h1>
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
          {t("aiInviteIntro", "A business owner has invited you to BonBox with read-only access to their books. Choose a password to finish setting up your accountant account.")}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
              {t("aiYourNameOptional", "Your name (optional)")}
            </label>
            <input
              type="text"
              value={fullName}
          ...
          >
            {submitting ? t("aiSettingUpAccount", "Setting up your account…") : t("aiAcceptInvitation", "Accept invitation")}
          </button>
        </form>

        <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-5 leading-relaxed">
          {t("aiReadOnlyNote", "You'll have read-only access. You can view reports and books but never modify data. The business owner can revoke your access at any time.")}
        </p>
      </div>
    </div>
  );
