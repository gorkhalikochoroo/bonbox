"""Analytics-telemetry endpoints — moved from routers/events.py in the
kulturarrangør sprint (migration 013) to free the `/api/events` namespace
for the new Event entity CRUD (cultural events).

This router handles the page_view / feature-usage telemetry that the
useEventLog + useSmartTelemetry hooks fire. The DB model is `EventLog`
(in app/models/event_log.py) and the wire format is unchanged — only the
URL prefix moved (from `/api/events` to `/api/event-log`).

The legacy `/api/events` POST + GET endpoints stay live as deprecated
aliases (registered separately in main.py) for native-app builds that
ship with the old URL baked in. New clients should call `/api/event-log`.
"""
